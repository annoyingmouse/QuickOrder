...
const createButton = (text, hide = false) => {
...
const prep_modal = (modal) => {
  let pageTrack = 0
  const backButton = createButton('Back', true)
...
(() => {
  ...
  panels.forEach(panel => {
    panelContainer.insertAdjacentHTML('beforeend', `
      <div class="col-6 col-lg-3 col-md-4 col-sm-6 p-2">
        <div class="card">
          <img src="https://via.placeholder.com/150" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title text-center">${panel.name}</h5>
            <p class="card-text truncated-3 tooltip-test" title="${panel.description}">
              ${panel.description}
            </p>
          </div>
        </div>
  ...
  layouts.forEach(layout => {
    layoutContainer.insertAdjacentHTML('beforeend', `
      <div class="col-6 col-lg-3 col-md-4 col-sm-6 p-2">
        <div class="card">
          <img src="${layout.image}" class="card-img-top" alt="${layout.name}">
          <div class="card-body">
            <h5 class="card-title text-center">${layout.name}</h5>
          </div>
        </div>
      </div>
    `)
